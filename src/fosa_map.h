...
static size_t fosa_map_hash(const char* cp, const size_t size) {
...
static const size_t fosa_map_find_index(fosa_map_t* map, const char* key) {
...
void fosa_map_resize(fosa_map_t* map, const size_t size) {
...
void fosa_map_init(fosa_map_t* map, const size_t size) {
    map->size = 0;
    map->count = 0;
    map->data = NULL;
...
void fosa_map_put(fosa_map_t* map, const char* key, void* value) {
...
void* fosa_map_get(fosa_map_t* map, const char* key) {
    return map->data[fosa_map_find_index(map, key)].value;
}

void fosa_map_delete(fosa_map_t* map, const char* key) {
    fosa_map_kv_t* kv = &map->data[fosa_map_find_index(map, key)];
    if(kv->key != NULL) {
        kv->key = NULL;
        kv->value = NULL;
